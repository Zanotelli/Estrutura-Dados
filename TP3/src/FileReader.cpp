...
void writeHeader(FILE *f) {//Table mapping 'codewords' to actual symbols
    symCode record;
    node *p;
    int temp=0,i=0;
    p=HEAD;
...
void writeBit(int b,FILE *f) {//My Logic: Maintain static buffer, if it is full, write into file 
...
void insert(node *p,node *m) { // insert p in list as per its freq., start from m to right,
...
void addSymbol(char c) {// Insert symbols into linked list if its new, otherwise freq++
    node *p,*q,*m;
    int t;
